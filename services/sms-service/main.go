...
var (
	smsUpstreamURL = getEnv("SMS_UPSTREAM_URL", "http://wiremock:8080")
...
func handleSendSMS(w http.ResponseWriter, r *http.Request) {
	...
	if sms.To == "" || sms.Message == "" {
		writeJSONError(w, "to and message are required", http.StatusBadRequest)
		return
	}
	...
	req, err := http.NewRequestWithContext(r.Context(), http.MethodPost, smsUpstreamURL+"/sms/send", bytes.NewReader(body))
	if err != nil {
	...
	if err != nil {
		slog.Error("Failed to call SMS provider", "error", err)
	...
	if _, err := io.Copy(w, resp.Body); err != nil {
		slog.Error("Failed to copy SMS provider response", "error", err)
	}
}

func main() {
	r := http.NewServeMux()
	r.HandleFunc("/sms/send", handleSendSMS)
	...
	slog.Info("SMS service starting", "port", port)
